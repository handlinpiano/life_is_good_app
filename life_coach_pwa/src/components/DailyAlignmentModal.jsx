...
export default function DailyAlignmentModal({ isOpen, onClose }) {
  ...
  const seedsQuery = useQuery(api.seeds.list, isAuthenticated ? {} : 'skip');
  const checkinsQuery = useQuery(api.checkins.list, isAuthenticated ? {} : 'skip');
  const seeds = useMemo(() => seedsQuery ?? [], [seedsQuery]);
  const checkins = useMemo(() => checkinsQuery ?? [], [checkinsQuery]);
  const upsertCheckin = useMutation(api.checkins.upsertByDate);
  ...
  const loadAlignment = async () => {
    ...
    try {
      ...
      const seedsWatered = seeds.filter((s) =>
  ...
  const loadAllGuidance = async (alignData) => {
    const loadingState = {};
    activeGurus.forEach((id) => {
      loadingState[id] = true;
    });
    setLoadingGurus(loadingState);

    const promises = activeGurus.map((guruId) =>
      fetchGuruGuidance(guruId, alignData)
    );
  ...
  const fetchGuruGuidance = async (guruId, alignData) => {
    ...
    try {
      const guruSeeds = seeds.filter((s) =>
        guru.seedCategories.includes(s.category)
      );
      const seedList =
        guruSeeds
          .map((s) => {
...
My seeds in your domain: ${seedList}`;
      ...
      setGuruGuidance((prev) => ({
        ...prev,
        [guruId]: 'Unable to connect today.',
      }));
    } finally {
                ...
                {showHistory ? (
                  <>
                    <button
                      ...
                      className="hover:bg-white/20 rounded-full p-1 -ml-1"
                    >
                      <ChevronLeft size={20} />
                    </button>
                    History
                  ...
                  <>
                    <Sparkles className="text-yellow-300" size={22} /> Daily
                    Alignment
                  </>
                )}
              </h2>
              ...
              <div className="space-y-2">
                {sortedCheckins.length > 0 ? (
                  sortedCheckins.map((checkin) => {
                    ...
                    const watered =
                      checkin.seedsWatered ?? checkin.seeds_watered ?? 0;
                    const total =
                      checkin.seedsTotal ?? checkin.seeds_total ?? 0;
                    return (
                      <div
                        key={checkin._id || checkin.localId || checkin.date}
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-xs text-stone-500 dark:text-stone-400 mt-0.5">
                              {checkin.panchang?.tithi || '—'} ·{' '}
                              {checkin.panchang?.nakshatra || '—'}
                            </div>
                          </div>
              ...
              {alignment.tithi.special === 'Ekadashi' && (
                <div className="bg-amber-100 dark:bg-amber-900/30 p-3 rounded-xl border border-amber-200 dark:border-amber-800">
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
                    An auspicious day for fasting, spiritual practice, and
                    introspection.
                  </p>
                </div>
              )}
