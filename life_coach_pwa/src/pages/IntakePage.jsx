...
export default function IntakePage() {
  ...
  const { user: authUser, saveProfile, profile, profileReady } = useAuth();
  const navigate = useNavigate();

  // Redirect if profile already has a chart (Convex is source of truth)
  useEffect(() => {
    if (profileReady && (profile?.chartData || chart)) {
      navigate('/dashboard');
    }
  }, [profileReady, profile, chart, navigate]);
  ...
  const handleSubmit = async (data) => {
    ...
    if (success) {
      ...
      if (saveResult?.success) {
        ...
        console.warn('Offline save failed, proceeding with local chart cache');
        navigate('/dashboard');
      } else {
